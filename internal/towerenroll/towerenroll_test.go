...
func TestAnApprovedOperatorEnrollsByProvingItsLocalKey(t *testing.T) {
	...
	require.Equal(t, toweradmit.StateQuarantine, res.Tower.State)
...
func TestNewRefusesAnIncompleteConfiguration(t *testing.T) {
	reg := toweradmit.NewWithStore(toweradmit.Config{}, toweradmit.NewMemStore())
	auth, err := towercert.NewAuthority(towercert.Config{})
	require.NoError(t, err)
...
func TestAnExpiredTokenIsRefused(t *testing.T) {
	...
	reg := toweradmit.NewWithStore(toweradmit.Config{TokenTTL: 20 * time.Millisecond}, toweradmit.NewMemStore())
	auth, err := towercert.NewAuthority(towercert.Config{TTL: time.Hour})
	require.NoError(t, err)
	e, err := New(Config{
