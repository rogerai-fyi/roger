...
// TestBandStoreCRUD covers the Mem band store: create, the three lookups
// (by code hash / by node / by owner), owner-scoped revoke, and the active-count
// cap helper. Mirrors grant_test.go's coverage.
func TestBandStoreCRUD(t *testing.T) {
	m := NewMem()
	now := time.Now()
	b := Band{ID: "band_1", CodeHash: "h1", CodeDisplay: "147.520 MHz · 8F3K-9M2Q",
		Owner: "owner1", NodeID: "n1"}
	...
	if len(list) != 1 {
		t.Errorf("BandsByOwner = %d, want 1", len(list))
	}
