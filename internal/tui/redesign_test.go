...
func TestSignalGrading(t *testing.T) {
	...
	raw := signalBarsRaw(1, 0, 700, true, 0, 4) // no broker signal -> tps fallback: fast + 4 stations -> tall tower
	peak := false
	for _, r := range raw {
		if r == '▇' || r == '█' {
			peak = true
	...
	// one at the same tps.
	one := signalBarsRaw(1, 0, 60, true, 0, 1)
	many := signalBarsRaw(1, 0, 60, true, 0, 4)
	if one == many {
