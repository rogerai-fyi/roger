...
func initializeOperatorScenarios(t *testing.T, st *opBDD, sc *godog.ScenarioContext) {
	...
	sc.Step(`^the (opencode|hermes|aider|pi) launch is materialized$`, st.materialize)
	sc.Step(`^the (opencode|hermes|aider|pi) launch is materialized and the guest exits$`, st.materializeAndExit)
	sc.Step(`^the (opencode|hermes|aider|pi) launch is materialized again$`, st.materializeAgain)
	...
	sc.Step(`^the pi config contains the session key$`, st.piConfigCarriesKey)
	sc.Step(`^the pi config file has mode 0600$`, st.piConfigMode0600)
	sc.Step(`^the "([^"]*)" entry is gated behind operator setup$`, st.entryIsGated)
