...
// through towerinv.Policy. Until now nothing implemented that interface, which meant the
...
// and the reason it is written as it is. towerinv.Policy has no error returns - Station
...
// Stations is the attachment registry read. Implemented by *stationattach.Registry.
type Stations interface {
	Station(stationID string) (stationattach.Attachment, bool, error)
}
...
// Policy implements towerinv.Policy.
type Policy struct {
	stations Stations
	bans     Bans
	owners   Owners
...
func (p *Policy) Station(stationID string) towerinv.Registration {
	...
	if !ok {
		return towerinv.Registration{Unavailable: true}
	}
	...
	reg := towerinv.Registration{
		Known:      true,
		Key:        ed25519.PublicKey(key),
		Banned:     banned || ownerBanned,
		KeyRevoked: at.State == stationattach.StateRevoked,
	}
	...
	// towerinv has.
	if at.State == stationattach.StateDetached {
		reg.Banned = true
	}
