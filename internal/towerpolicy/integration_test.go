...
//
// A compile-time assertion that *Policy satisfies towerinv.Policy is not enough: an
// implementation can satisfy an interface and still answer every question wrongly.
var _ towerinv.Policy = (*Policy)(nil)
...
func newChain(t *testing.T) *chain {
	...
	astore := stationattach.NewMemStore()
	reg := stationattach.New(stationattach.Config{
	...
	set := towerinv.New(towerinv.Config{
		Network: netName, Now: func() time.Time { return now },
	}, pol)
...
func (c *chain) inventory() []byte {
	...
	signedLeaf, err := towerobj.Sign(c.stPriv, netName, towerinv.TypeOffer, towerinv.Version, raw, "station_sig")
	require.NoError(c.t, err)
	...
	require.NoError(c.t, err)
	signedInv, err := towerobj.Sign(c.towerPriv, netName, towerinv.TypeInventory, towerinv.Version, rawInv, "sig")
	require.NoError(c.t, err)
	return signedInv
}

func (c *chain) accept() (towerinv.Result, error) {
