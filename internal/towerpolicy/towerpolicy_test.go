...
//
// towerinv.Policy has no error returns, so a read that fails has nowhere to report itself.
...
type fakeStations struct {
	at  stationattach.Attachment
	ok  bool
	err error
}

func (f *fakeStations) Station(string) (stationattach.Attachment, bool, error) {
...
func liveAttachment(t *testing.T) (stationattach.Attachment, ed25519.PublicKey) {
	t.Helper()
	pub, _, err := ed25519.GenerateKey(nil)
	require.NoError(t, err)
	return stationattach.Attachment{
		StationID:    stationID,
		Owner:        ownerPub,
		AssertionKey: hex.EncodeToString(pub),
		Origin:       stationattach.Origin{Kind: stationattach.OriginJoined, TowerID: "tw-1"},
		State:        stationattach.StateQuarantine,
	}, pub
}
...
func TestAnUnreadableOrUnknownStationIsNotKnown(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"the stored key is not hex", &fakeStations{ok: true, at: stationattach.Attachment{
			StationID: stationID, Owner: ownerPub, AssertionKey: "not-hex"}}},
		{"the stored key is the wrong length", &fakeStations{ok: true, at: stationattach.Attachment{
...
func TestLifecycleStatesMapOntoTheFieldsTowerinvHas(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{stationattach.StateQuarantine, false, false},
		{stationattach.StateActive, false, false},
		{stationattach.StateRevoked, true, false},
		...
		{stationattach.StateDetached, false, true},
	} {
