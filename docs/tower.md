...
like any other earning: Stripe Connect transfer, the 30-day hold with its 90-day reserve tail, $25 minimum, KYC gate, and
...
| Audit leniency for new Stations | built — a Station that has never answered a content audit gets a soft miss rather than a quarantine; the leniency ends per Station the first time it produces a verifying transcript, so no flag day is needed |
