...
#
# PROVEN-LIVE (the pick gate, active probe ENABLED) = the node has a PASSED canary with
# a clean failure streak (verifiedServing) AND that proof is FRESH (a canary pass OR a
# real served request within the probe ceiling; measurementStale=false). When the active
