...
| `giga-bot-{roll,inspect,idle}-*` | THE GIGA UNIT: a Victorian automaton inspection robot - barrel body on spoked wheels, single lens eye; rolling / examining with magnifier and clipboard / at rest with a pennant, no red | various | see files | 7 | Giga embodied on the factory floor - the plant's visible mind |
| `game-{mixer,oven,packer}-mk2-*` / `-mk3-*` | The cookie line's UPGRADED machinery: Mk II adds size and instrumentation (banded bowl + gauges / long twin-door oven / tall twin-chute press), Mk III is the industrial form (twin-bowl gantry mixer / tunnel oven with conveyor pass-through / multi-lane gantry packer), no red | various | see files | 7 | Swapped in when a machine tier upgrades - the upgrade must be SEEN |
