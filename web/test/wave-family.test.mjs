...
test("the markets section names the constraint instead of alluding to it", () => {
  ...
  // It must not read as a claim of deployments we do not have.
  assert.match(copy, /not deployments we are claiming|patterns we design for|not customer/i,
    "these are patterns, not customers");
});
