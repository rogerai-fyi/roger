...
test("two ink zones: the console + the band, then monetize + privacy + go", () => {
  const z = zones();
  assert.equal(z.length, 2);
  assert.match(z[0], /<section class="demo" id="demo">[\s\S]*<section class="market" id="market">/);
  assert.doesNotMatch(z[0], /id="company"/, "the company section is back on paper");
  assert.match(z[1], /id="monetize"[\s\S]*id="operator"[\s\S]*<section class="cta">/);
});
...
test("zone text stays AA (4.5:1) on the zone ground AND under the bloom at its peak, light and dark site", () => {
  ...
  for (const [name, z] of [["light site", lightZone], ["dark site", darkZone]]) {
    for (const ground of [z["--paper"], z["--paper-2"]]) {
      const bloomed = over(live, peak, ground);
