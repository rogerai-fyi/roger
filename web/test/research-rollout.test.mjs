...
test("(b) figure, scroll box, code block and tinted panel are components.css components", () => {
  const blocks = componentBlocks();
  for (const b of ["figure", "scroll-box", "code-block", "tint-panel"]) assert.ok(blocks.has(b), `components.css defines .${b}`);
  ...
  for (const h of ["### Figure", "### Code block", "### Scroll box", "### Tinted panel"]) assert.ok(doc.includes(h), `DESIGN-SYSTEM.md documents ${h}`);
});
...
test("(c) every article sign-off is an inset panel; a closing install command is the shared pill", () => {
  ...
  for (const page of ARTICLES) {
    ...
    panels += (html.match(/<aside class="tint-panel bc-signoff"/g) || []).length;
...
test("index: the founder telegram folds on a phone and stays whole without JS", () => {
  ...
  assert.match(html, /<details class="bc-tg tint-panel" data-fold-narrow open/, "a disclosure, open in the HTML");
