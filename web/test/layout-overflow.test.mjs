...
test("commands: every multi-word command in a sign-off note breaks only at spaces", () => {
  ...
  assert.equal(decl(css("base.css"), ".cmd-words > span")["white-space"], "nowrap");
});
...
test("home: the hero column may shrink below its longest word", () => {
  ...
  const title = decl(c, ".hero__title", (m) => maxWidth(m) <= 480);
  assert.match(title["font-size"] || "", /vw/,
    "on a phone the headline scales with the viewport so the nowrap underlined phrase fits");
});
